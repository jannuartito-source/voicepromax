...
## Login & mengatur user (users.txt)
Login diatur lewat file **`users.txt`** — bisa kamu ubah kapan saja.

- Format tiap baris: `username:password` (satu user per baris).
- Baris diawali `#` = komentar (diabaikan). Baris kosong diabaikan.
- **Simpan file → langsung berlaku, TIDAK perlu restart server.** (Menambah/menghapus user cukup edit + simpan.)
- Kalau semua user dihapus/dikomentari → web jadi **terbuka** (tanpa login).

Contoh isi `users.txt`:
```
admin:rahasia123
budi:passwordbudi
siti:12345
...
Ganti dulu password default `admin:rahasia123` dengan milikmu. Kalau kamu push ke GitHub, jangan taruh password asli di sana (atau tambahkan `users.txt` ke `.gitignore`).
