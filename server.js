...
// --- SISTEM LOGIN USER (users.txt) ---
// File users.txt dibaca ULANG setiap login, jadi kamu bisa tambah/ubah/hapus user
// kapan saja tanpa perlu restart server. Format tiap baris:  username:password
// Baris kosong & baris diawali # diabaikan (komentar).
function readUsers() {
  const users = new Map();
...
// Login aktif hanya jika ada minimal satu user. users.txt kosong = web terbuka (tanpa login).
function loginEnabled() { return readUsers().size > 0; }

// Sesi disimpan di memori. Server restart = semua sesi hangus (tinggal login lagi).
...
function guard(req, res, next) {
  if (!loginEnabled()) return next(); // tidak ada user -> web terbuka
  const s = validToken(req.get("x-auth-token"));
  if (s) { req.user = s.user; return next(); }
  return res.status(401).json({ error: "Sesi tidak valid. Silakan login.", needLogin: true });
}
...
app.get("/api/config", (_req, res) => {
  res.json({ authRequired: loginEnabled(), keyConfigured: Boolean(API_KEY), model: MODEL });
});

// Login: cek username+password terhadap users.txt (dibaca ulang tiap kali).
app.post("/api/login", (req, res) => {
  const user = (req.body?.username || "").toString().trim();
  const pass = (req.body?.password || "").toString();
  const users = readUsers();
  if (!users.size) return res.json({ ok: true, token: "", open: true }); // tidak ada user = terbuka
  if (users.has(user) && users.get(user) === pass) {
    return res.json({ ok: true, token: createSession(user), username: user });
  }
